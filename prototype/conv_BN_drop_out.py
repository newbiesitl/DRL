...
def main():
    ...
    num_epoch = 6
    strides = (1, 1)
    for dat in t.transform_all(data_folder, grey_scale=False, batch_size=100 if not train_model else 55000):
        if train is None:
    ...
    if train_model:
        ...
        with open(os.path.join(model_folder, model_name + "_ae.json"), "w") as json_file:
            json_file.write(model_json)
        # serialize weights to HDF5
        ae.save_weights(os.path.join(model_folder, model_name + "_ae.h5"))
        print("Saved model to disk")

        # serialize model to JSON
        model_json = encoder.to_json()
        with open(os.path.join(model_folder, model_name + "_encoder.json"), "w") as json_file:
            json_file.write(model_json)
        # serialize weights to HDF5
        encoder.save_weights(os.path.join(model_folder, model_name + "_encoder.h5"))
        print("Saved model to disk")
